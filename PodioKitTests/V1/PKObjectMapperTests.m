...
- (void)testSingleObjectMapping {
  ...
  XCTAssertTrue([result isKindOfClass:[PKTestItem class]], @"Wrong class, got %@", [result class]);
  
  PKTestItem *item = (PKTestItem *)result;
  XCTAssertTrue([item.itemId intValue] == 1234, @"Item id does not match, got %d", [item.itemId intValue]);
  XCTAssertTrue([item.title isEqualToString:@"This is an item title"], @"Item title does not match, got %@", item.title);
}
...
- (void)testCollectionMapping {
  ...
  id result = [mapper performMappingWithData:data];
  XCTAssertTrue([result isKindOfClass:[NSArray class]], @"Wrong class, got %@", [result class]);
  XCTAssertTrue([result count] == 2, @"Result should contain 2 items, got %d", [result count]);
  
  PKTestItem *item1 = (PKTestItem *)[result objectAtIndex:0];
  PKTestItem *item2 = (PKTestItem *)[result objectAtIndex:1];
  
  XCTAssertTrue([item1.itemId intValue] == 1234, @"Item id does not match, got %d", [item2.itemId intValue]);
  XCTAssertTrue([item1.title isEqualToString:@"This is an item title 1"], @"Item title does not match, got %@", item1.title);
  
  XCTAssertTrue([item2.itemId intValue] == 4321, @"Item id does not match, got %d", [item2.itemId intValue]);
  XCTAssertTrue([item2.title isEqualToString:@"This is an item title 3"], @"Item title does not match, got %@", item2.title);
}
...
- (void)testValueMapping {
  ...
  XCTAssertTrue(item.itemId == nil, @"Item id should be nil");
  XCTAssertTrue(item.title == nil, @"Item title should be nil");
  ...
  XCTAssertTrue([item.itemId intValue] == 3333, @"Item id does not match, got %d", [item.itemId intValue]);
  ...
  XCTAssertTrue([item.title isEqualToString:@"This is an item title - Appending some text"], @"Item title does not match, got %@", item.title);
}
...
- (void)testOneToManyRelationshipMapping {
  ...
  XCTAssertTrue([item.fields count] == 2, @"Item field count does not match, got %d", [item.fields count]);
}
...
- (void)testOneToOneRelationshipMapping {
  ...
  [mapper applyRelationshipMapping:mapping attributeValue:data object:item];
  XCTAssertTrue(item.app != nil, @"App should not be nil");
  XCTAssertTrue([item.app.appId intValue] == 4444, @"App id does not match, got %d", [item.app.appId intValue]);
  XCTAssertTrue([item.app.name isEqualToString:@"Bugs"], @"App title does not match, got %@", item.app.name);
}
