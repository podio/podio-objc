//
//  PKTEmojiValueTransformer.m
//  PodioKit
//
//  Created by Pavel Prochazka on 06/09/15.
//  Copyright © 2015 Citrix Systems, Inc. All rights reserved.
//

#import "PKTEmojiValueTransformer.h"

static NSDictionary * kEmojiTable = nil;

@implementation PKTEmojiValueTransformer
...
- (id)transformedValue:(id)value {
  ...
  NSString *nonEmojifiedString = (NSString *)value;
  NSString *emojifiedString = [self emojifiedStringFromString:nonEmojifiedString error:nil];
...
- (NSString *)emojifiedStringFromString:(NSString *)string error:(NSError **)error {
  ...
  NSDictionary *emojiTable = [self emojiTable];
  NSMutableString *emojifiedString = [string mutableCopy];
  
  NSArray *results = [expression matchesInString:string options:0 range:NSMakeRange(0, string.length)];
  for (NSTextCheckingResult *result in [results reverseObjectEnumerator]) {
    if (result.numberOfRanges > 1) {
      NSRange nameRange = [result rangeAtIndex:1]; // The first capture group contains the emoji name without ':'
      NSString *emojiName = [string substringWithRange:nameRange];
      NSString *emojiCode = emojiTable[emojiName];
      
      if (emojiCode) {
        [emojifiedString replaceCharactersInRange:result.range withString:emojiCode];
      }
    }
  }
  
  return [emojifiedString copy];
}

- (NSDictionary *)emojiTable {
