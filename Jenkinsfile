    ...
    stages {
        ...
        stage('Polaris') {
            environment {
                ...
                BRIDGE_POLARIS_PROJECT_NAME = "podio-objc"
        ...
        stage('BlackDuckSCA') {
            environment {
                ...
                BRIDGE_DETECT_ARGS = "--detect.project.name=DX-Podio-podio-objc --detect.project.version.name=${env.branchName} --detect.project.version.update=true --detect.project.version.distribution=SAAS --detect.project.group.name=Podio-Podio"
            }
            steps {
                dir('podio-objc') {
                    script {
        ...
        stage('TruffleHog Scan') {
            steps {
                script {
                    ...
                    def repoPath = "${WORKSPACE}/podio-objc"
